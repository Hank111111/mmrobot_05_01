...
// modified from https://github.com/Yiphy/Ransac-2d-Shape-Detection
...
namespace sac
{
	...
	int ransacModelEllipse2D::countWithinDistance(const ModelCoefficient model_coefficients, const double threshold)
	{
		...
		double f1x = cx - cA*cos(angle*M_PI / 180);
		double f1y = cy - cA*sin(angle*M_PI / 180);
		double f2x = cx + cA*cos(angle*M_PI / 180);
		double f2y = cy + cA*sin(angle*M_PI / 180);
...
cv::RotatedRect RansacFitEllipse(std::vector<cv::Point> points, std::vector<int>& inliers_indexes){
	std::vector<sac::Point2D> sac_points;
	sac_points.insert(sac_points.begin(), points.begin(), points.end());

	sac::ransacModelEllipse2D ellipse2D;
	sac::ModelCoefficient parameter;
	ellipse2D.setDistanceThreshold(2);
	ellipse2D.setMaxIterations(25);


	ellipse2D.setInputCloud(sac_points);
	if(!ellipse2D.computeModel()) return cv::RotatedRect();
	ellipse2D.getInliers(inliers_indexes);
	...
	return cv::RotatedRect(ellipseCenter, ellipseSize, ellipseAngle);
