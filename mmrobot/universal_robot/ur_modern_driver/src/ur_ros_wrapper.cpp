...
private:
	...
	void publishMbMsg() {
		...
		ros::Publisher protected_pub = nh_.advertise<std_msgs::String>(
				"ur_driver/protected_states", 1);
			...
			if (robot_.sec_interface_->robot_state_->isEmergencyStopped()
				...
				if (robot_.sec_interface_->robot_state_->isEmergencyStopped()
					...
					print_error("Emergency stop pressed!");
						...
						and !warned) {
					print_error("Robot is protective stopped!sds");
					//输出到topic
					std_msgs::String ccc;
 					ccc.data= "Robot is protective stopped!sds";
					protected_pub.publish(ccc);
				}
				if (has_goal_) {
